...
# Clean extracts live in WORKING/ (auto-generated by the extract scripts; NOT in
...
# Curated reasons for the hardcoded-utilisation note shown in tweak mode (user-
# supplied, 2026-06-20). Keyed by main name; any hardcoded project not listed here
# falls back to the generic wording. Detection is automatic (see extract: 'HC
# months' + Assumptions "Hard coded"); only these explanations are curated.
HC_REASONS = {
    "Cameroon FLNG": "Set to cease operations in July 2026.",
    "Snohvit LNG": "Reduced for annual maintenance.",
    "Ras Laffan": "Disruption following the US–Iran conflict.",
    "Arctic LNG 2 T1-2": "No production during the winter months.",
    "Arctic LNG 2 T3": "Assumed offline due to sanctions.",
    "Arctic LNG 1 T1-3": "Assumed offline due to sanctions.",
    "Atlantic LNG": "Reduced for a maintenance shutdown.",
    "Das Island": "Disruption following the US–Iran conflict.",
    "Calcasieu Pass LNG": "Set to the company’s production guidance.",
    ...
    try:
        ...
        for r in range(2, ws.max_row + 1):
            ...
            for i, p in enumerate(partners):
                ...
                if s is not None:
                    co_stakes[p] = s
            # Hardcoded-utilisation month mask (col 23, comma-joined indices).
            hc_raw = str(cell(23) or "").strip()
            hc = [int(x) for x in hc_raw.split(",") if x.strip().isdigit()] if hc_raw else []
            # A project is "hardcoded" if it has fixed months OR a flat hardcoded
            # level (Assumptions util_forecast == "Hard coded" -> emitted as a string).
            uf = cell(10)
            is_hardcoded = bool(hc) or isinstance(uf, str)
            proj = {
            ...
            if is_hardcoded:
                proj["hc_note"] = HC_REASONS.get(proj["name"], HC_REASON_GENERIC)
        ...
        # Scenario params (for the interactive tweak engine): per-project month
        # indices for start (first producing month) and decline-start, on the
        # monthly axis. History (<= latest_actual) is frozen; tweaks recompute only
        # the forecast and re-aggregate views from the monthly series via "idx".
        mv = production["views"]["Monthly"]
        ym_to_idx = {(c["year"], c.get("month")): i for i, c in enumerate(mv["col_meta"])}
        prod_monthly = {row["label"]: row["base"] for row in mv["rows"]}
        for p in projects:
            base = prod_monthly.get(p["name"], [])
            p["start_idx"] = next((i for i, v in enumerate(base) if isinstance(v, (int, float)) and v > 1e-9), -1)
            ds = str(p.get("decline_start") or "")
...
def build_projects_production(wb, latest_actual=None):
    ...
    for view_name, period_cols in period_results.items():
        ...
        views[view_name] = {
            ...
            # Monthly indices per bucket — lets the front-end re-aggregate this view
            # from the monthly series after a scenario tweak (interactive recompute).
            "idx": [c["indices"] for c in period_cols],
            "rows": [{"label": a["label"], "base": a["base_values"]} for a in aggregated],
        }
...
def build_projects_capacity(wb, latest_actual=None):
    ...
    period_results = aggregate_monthly_to_periods(dates, days)
    # "Monthly" is built but NOT shown on the Capacity tab — it's the monthly source
    # the scenario engine reads (and re-aggregates the 4 shown periods from).
    WANTED = ["Monthly", "Annual CY", "Gas Year", "Winter", "Summer"]
    ...
    def build_views(rows):
        ...
        for vn in WANTED:
            ...
            views[vn] = {
                ...
                "idx": [c["indices"] for c in pc],     # monthly indices per bucket (scenario recompute)
...
# HTML GENERATION
# ============================================================
def _round_floats(o, nd=6):
    """Recursively round every float in a nested dict/list to `nd` decimals so the
    embedded JSON blob is compact. Leaves ints, bools, strings and None untouched;
    passes NaN/inf through unchanged (json handles them)."""
    if isinstance(o, bool):
        return o
    if isinstance(o, float):
        if o != o or o in (float("inf"), float("-inf")):
            return o
        return round(o, nd) + 0.0  # +0.0 collapses -0.0 -> 0.0
...
def generate_html(datasets_by_key, ordered_keys, assets):
    ...
    # Size trim: the embedded blob is dominated by ~190k per-project monthly floats
    # carried at full 15-17 digit precision (production / capacity / utilisation +
    # weights). Round every float to 6 dp before serialisation — invisible at the
    # display level (sub-0.0001 mmt / 0.0001% util) and keeps the cap x util / 12
    # identity intact, while cutting ~10 chars off each value. Ints/None untouched.
    data_json = json.dumps(_round_floats(master), separators=(",", ":"))
...
/* Overview header bars: a light tint distinguishing imports vs exports sections. */
.growth-controls.lng-head-imports { background: #eef2fb; }   /* imports = light blue */
...
/* ---- Scenario (interactive model tweaks) ---- */
.scen-bar {
    display: none; align-items: center; gap: 16px; flex-wrap: wrap; justify-content: flex-start;
    margin: 0 20px 12px; padding: 8px 16px; border-radius: 10px;
    background: #fdf3e7; border: 1.5px solid #e0a341;
}
.scen-bar.scen-clean { background: """ + card + """; border-color: """ + border + """; }
.scen-status { display: flex; align-items: center; gap: 14px; margin-left: auto; }
.scen-pill { position: relative; cursor: default; padding: 4px 10px; border-radius: 999px;
    background: rgba(192,0,0,0.08); border: 1px solid rgba(192,0,0,0.25); }
.scen-bar.scen-clean .scen-pill { background: #fff; border-color: """ + border + """; }
.scen-changes-pop {
    display: none; position: absolute; top: 100%; right: 0; left: auto; margin-top: 8px; z-index: 50;
    min-width: 250px; max-width: 360px; padding: 12px 14px; text-align: left;
    background: #fff; border: 1px solid """ + border + """; border-radius: 10px;
    box-shadow: 0 8px 28px rgba(39,41,98,0.18); font-weight: normal;
}
.scen-pill:hover .scen-changes-pop, .scen-pill:focus .scen-changes-pop, .scen-pill:focus-within .scen-changes-pop { display: block; }
/* Invisible bridge over the 8px gap so moving the cursor from the pill into the
   white box keeps it open; the box hides only when the cursor leaves both. */
.scen-changes-pop::before { content: ''; position: absolute; top: -10px; left: 0; right: 0; height: 10px; }
.scen-pop-title { font-size: 10px; text-transform: uppercase; letter-spacing: 0.6px; color: """ + grey + """; margin-bottom: 8px; font-weight: bold; }
/* Flex rows (NOT a table): the name column can shrink/wrap via min-width:0 so the
   tag + value columns stay fixed-width and never push out of the white box. */
.scen-pop-row { display: flex; align-items: baseline; gap: 10px; padding: 6px 0; border-top: 1px solid #f0f1f5; }
.scen-pop-row:first-of-type { border-top: none; }
.scen-pop-k { flex: 1 1 auto; min-width: 0; color: """ + db + """; font-weight: bold; font-size: 12px; word-break: break-word; }
.scen-pop-t { flex: 0 0 auto; color: """ + grey + """; font-size: 10px; text-transform: uppercase; letter-spacing: 0.3px; text-align: left; }
.scen-pop-v { flex: 0 0 56px; color: """ + db + """; font-weight: bold; font-size: 12px; text-align: right; white-space: nowrap; font-variant-numeric: tabular-nums; }
/* Reset confirmation modal */
.scen-modal-overlay { position: fixed; inset: 0; background: rgba(20,22,40,0.45); display: none; align-items: center; justify-content: center; z-index: 1000; }
.scen-modal { background: #fff; border-radius: 12px; padding: 22px 24px; max-width: 380px; box-shadow: 0 14px 44px rgba(0,0,0,0.30); }
.scen-modal-title { font-size: 15px; font-weight: bold; color: """ + db + """; margin-bottom: 10px; }
.scen-modal-body { font-size: 12.5px; color: """ + db + """; line-height: 1.45; margin-bottom: 18px; }
.scen-modal-actions { display: flex; justify-content: flex-end; gap: 10px; }
.scen-modal-no, .scen-modal-yes { font-family: 'Gotham Book','Segoe UI',Calibri,sans-serif; font-size: 12px; font-weight: bold; padding: 8px 16px; border-radius: 8px; cursor: pointer; }
.scen-modal-no { background: #fff; border: 1.5px solid """ + border + """; color: """ + db + """; }
.scen-modal-no:hover { border-color: """ + db + """; }
.scen-modal-yes { background: """ + red + """; border: 1.5px solid """ + red + """; color: #fff; }
...
.scen-adjust-btn {
    font-family: 'Gotham Book','Segoe UI',Calibri,sans-serif; font-size: 12px; font-weight: bold;
    padding: 8px 18px; border-radius: 8px; border: 1.5px solid """ + db + """;
    background: #fff; color: """ + db + """; cursor: pointer; transition: all 0.15s;
}
.scen-adjust-btn.active, .scen-adjust-btn:hover { background: """ + db + """; color: #fff; }
.scen-adjust-controls { display: flex; align-items: flex-end; gap: 18px; flex-wrap: wrap; }
.scen-sens { display: flex; flex-direction: column; gap: 4px; }
.scen-sens label { font-size: 10px; font-weight: bold; text-transform: uppercase; color: """ + grey + """; letter-spacing: 0.5px; }
.scen-sens-row { display: flex; align-items: center; gap: 8px; }
.scen-sens-row input[type=range] { width: 200px; accent-color: """ + db + """; }
.scen-sens-row input[type=number] { width: 60px; padding: 5px 6px; border: 1.5px solid rgba(39,41,98,0.25); border-radius: 6px; font-family: inherit; font-size: 12px; color: """ + db + """; }
.scen-sens-unit { font-size: 11px; color: """ + grey + """; }
.scen-reset-btn {
    font-family: 'Gotham Book','Segoe UI',Calibri,sans-serif; font-size: 11.5px; font-weight: bold;
    padding: 7px 16px; border-radius: 8px; border: 1.5px solid """ + red + """;
    background: #fff; color: """ + red + """; cursor: pointer; align-self: flex-end;
}
...
.scen-modified { font-size: 11px; color: """ + red + """; margin-top: 6px; }
.scen-revert { font-size: 11px; color: """ + db + """; background: none; border: none; text-decoration: underline; cursor: pointer; padding: 0 2px; }
.scen-mark { color: """ + red + """; font-size: 12px; }
.prj-table tr.scen-row-edited td { background: #fff7ec; }
.prj-stat .scen-inp, .prj-stat .scen-sel { margin-left: 2px; }
...
function updateDatasetUI() {
    ...
    if(typeof scenUpdateBars==='function') scenUpdateBars();   // show/hide the scenario bar
}
...
var LNG_EXP_CACHE = null;
function lngExportsInvalidate(){ LNG_EXP_CACHE = null; }
function lngSubBlob(k) {
    if (k === 'exports') { if(!LNG_EXP_CACHE) LNG_EXP_CACHE = lngExportsSub(); return LNG_EXP_CACHE; }
    return LNG.sub[k];
}
// Overview EXPORTS sourced from the PROJECTS production (same source as Supply
// Outlook, scenario-aware via the swapped lng_projects production). Keeps the
// original export hierarchy/chart structure; only the row VALUES are overridden by
// aggregating the projects by region (parent rows) / country (child rows).
function lngExportsSub(){
    var base = LNG.sub.exports;
    var LP = (typeof ALL_DATASETS!=='undefined') ? ALL_DATASETS.lng_projects : null;
    if(!base || !LP || !LP.production) return base;
    var regionMembers={}, countryMembers={}, allNames=[], regionSet={};
    (LP.region_order||[]).forEach(function(r){ regionSet[r]=1; });
    LP.projects.forEach(function(p){
        allNames.push(p.name);
        (regionMembers[p.region]=regionMembers[p.region]||[]).push(p.name);
        (countryMembers[p.country]=countryMembers[p.country]||[]).push(p.name);
    });
    var out={}; for(var kk in base) out[kk]=base[kk]; out.views={};
    for(var per in base.views){
        var bv=base.views[per], pv=LP.production.views[per];
        if(!pv || pv.col_meta.length!==bv.col_meta.length){ out.views[per]=bv; continue; }
        var prodByName={}; pv.rows.forEach(function(r){ prodByName[r.label]=r.base; });
        var ncol=bv.col_meta.length;
        var aggFn = function(names){ var a=new Array(ncol); for(var i=0;i<ncol;i++) a[i]=0;
            for(var nI=0;nI<names.length;nI++){ var b=prodByName[names[nI]]; if(b) for(var i=0;i<ncol;i++) a[i]+=b[i]||0; } return a; };
        var nv={}; for(var k2 in bv) nv[k2]=bv[k2];
        nv.rows = bv.rows.map(function(r){ var lbl=r.label, b;
            if(lbl==='Grand total') b=aggFn(allNames);
            else if(regionSet[lbl]) b=aggFn(regionMembers[lbl]||[]);   // region rows
            else b=aggFn(countryMembers[lbl]||[]);                     // country rows
            return { label:r.label, base:b };
        });
        out.views[per]=nv;
    }
    return out;
...
function lngInit() {
    LNG = DATA;
    if(typeof scenInitData==='function') scenInitData();   // scenario also drives Overview exports
    lngExportsInvalidate();                                // rebuild exports from (current) projects source
    var k;
...
function prjAttr(s){ return prjEsc(s).replace(/"/g,'&quot;'); }   // safe for a double-quoted attribute
...
function prjInit(){
    ...
    scenInitData();    // capture original monthly sources for the scenario engine
...
function prjRowProject(p, exp){
    ...
    var ed = (scenMode==='scenario') ? (scenEdits[p.name]||{}) : {}, edited = Object.keys(ed).length>0;
    var cosD = (ed.cos!=null ? ed.cos : p.cos);
    var riskedD = (ed.cos!=null && typeof p.unrisked==='number') ? p.unrisked*ed.cos : p.risked;
    var startD = (ed.startIdx!=null && SCEN_MONTHS) ? SCEN_MONTHS[ed.startIdx] : prjStart(p.start);
    var mark = edited ? ' <span class="scen-mark" title="Adjusted in your scenario">&#9998;</span>' : '';
    return '<tr class="prj-row-project'+(edited?' scen-row-edited':'')+'" data-prj-x="p|'+prjEsc(p.name)+'">'
        + '<td><span class="prj-arrow'+(exp?' expanded':'')+'">&#9654;</span> '+prjEsc(p.name)+mark+'</td>'
        + '<td style="text-align:left;">'+prjBadge(p.status)+'</td>'
        + '<td>'+prjNum(prjConvCap(p.unrisked),1)+star+'</td><td>'+prjNum(prjConvCap(riskedD),1)+star+'</td>'
        + '<td>'+prjPct(cosD)+'</td><td>'+startD+'</td>'
...
function prjRowDetail(p){
    ...
    // ---- editable assumptions in Adjust mode (role-gated) ----
    // In Original view the tree shows the base model (no edits); edits show only in Changes view.
    var ed = (scenMode==='scenario') ? (scenEdits[p.name]||{}) : {};
    var hardUtil = (typeof p.util_forecast === 'string');          // explicit "Hard coded"
    var isFuture = (p.status==='Under construction' || p.status==='Pre-FID');
    var isProd   = (p.status==='Producing');
    var canDecl  = isProd && !hardUtil && p.decline_start_idx>=0 && typeof p.util_decline==='number';
    var an = prjAttr(p.name), editF = scenEditMode && isFuture && !hardUtil;
    var startIdx = (ed.startIdx!=null ? ed.startIdx : p.start_idx);
    var cosCur   = (ed.cos!=null ? ed.cos : p.cos);
    var declCur  = (ed.declineRate!=null ? ed.declineRate : p.util_decline);
    var startV = editF
        ? '<select class="scen-sel" data-scen-edit="start" data-name="'+an+'">'+scenStartOptions(startIdx)+'</select>'
        : (ed.startIdx!=null && SCEN_MONTHS ? SCEN_MONTHS[startIdx] : prjStart(p.start));
    var cosV = editF
        ? '<input type="number" class="scen-inp" min="0" max="100" step="1" value="'+Math.round((cosCur||0)*100)+'" data-scen-edit="cos" data-name="'+an+'"> %'
        : prjPct(cosCur);
    var declV = (scenEditMode && canDecl)
        ? '<input type="number" class="scen-inp" step="0.1" value="'+((declCur||0)*100).toFixed(1)+'" data-scen-edit="decline" data-name="'+an+'"> %'
        : prjPct(p.util_decline);
    var hasHC = hardUtil || (p.hc && p.hc.length>0);
    var note='';
    if(scenEditMode){
        if(hasHC){
            var why = p.hc_note ? (' '+p.hc_note) : '';
            var lead = hardUtil
                ? 'This project&rsquo;s utilisation is fixed by Palissy Advisers and will not change.'
                : 'Some months of this project&rsquo;s utilisation are fixed by Palissy Advisers, so they stay put when you change the decline rate.';
            note='<div class="scen-note">'+lead+why+'</div>';
        }
        else if(p.status==='Shut-down') note='<div class="scen-note">Shut-down project &mdash; not adjustable.</div>';
    }
    var modified = Object.keys(ed).length>0
        ? '<div class="scen-modified">Adjusted from the base model <button class="scen-revert" data-name="'+an+'">revert</button></div>' : '';
    var stats = stat('Start', startV)
        + stat('Chance of success', cosV)
        + stat('Utilisation forecast', hardUtil ? 'Hard coded' : prjPct(p.util_forecast))
        + stat('Utilisation decline (annual)', declV)
    ...
    return '<tr class="prj-detail"><td colspan="7"><div class="prj-detail-inner">'
        ...
        + '<div class="prj-detail-col"><h4>Key assumptions</h4>'+note+'<div class="prj-stats">'+stats+'</div>'+modified+'</div>'
        + '</div></td></tr>';
}
function prjRenderTree(){
    ...
    document.getElementById('prjTableHead').innerHTML =
        ...
        + '<th>Risked ('+ul+')</th><th>Chance of success</th><th>Start</th><th>Operator</th></tr>';
    var byRegion={};
...
function prjBuildTree(viewBy, leaves, prod, vis, cellsFn){
    ...
    // Sort by a STABLE key (pristine full-period production), not the visible-range
    // total — so rows keep their position when the range changes or a tweak is applied.
    function tot(names){ var s=0; for(var n=0;n<names.length;n++) s+=(PRJ_SORTKEY[names[n]]||0); return s; }
...
/* ============================================================================
   SCENARIO ENGINE (interactive model tweaks). Per-project edits {cos,startIdx,
   declineRate} + a GLOBAL utilisation sensitivity (absolute % points). Each
   project's MONTHLY series is transformed over the FORECAST region only (history
   <= latest_actual is frozen); period views re-aggregate from monthly via each
   view's `idx` map. Built on the exact identity production = risked_cap x util /12.
   This block is the verified math core; view re-aggregation + UI wire on top.
   ========================================================================== */
var scenEdits = {};          // project name -> { cos?, startIdx?, declineRate? }
var scenUtilSens = 0;        // global utilisation sensitivity, fraction (+0.05 = +5pts)
var scenMode = 'original';   // current view: 'original' | 'scenario'
var SCEN_MON = null;         // original monthly sources, by project name
var SCEN_N = 0;              // number of months on the axis
var SCEN_LA = -1;            // latest-actual month index (forecast boundary)
var PRJ_PRISTINE = null;     // the untouched original production/capacity/utilisation
var PRJ_SCEN = null;         // recomputed scenario views (rebuilt on each edit)
var scenEditMode = false;    // Projects tab "Adjust assumptions" edit mode
var SCEN_MONTHS = null;      // monthly labels ("Jan 2029"...) for the start picker
var SCEN_MONTHS_SHORT = null;// short labels ("Jan-29") for the compact summary
var PRJ_SORTKEY = {};        // stable per-project order key (pristine full-period production)

function scenInitData(){
    // Operate on the lng_projects dataset directly (not the transient PRJ) so the
    // scenario also works when the Overview sub-tab is opened first. PRJ === this LP
    // when on the project sub-tabs, so swapping LP.* also drives those renderers.
    var LP = (typeof ALL_DATASETS!=='undefined') ? ALL_DATASETS.lng_projects : null;
    if(!LP || !LP.production) return;
    if(!PRJ_BY_NAME || !Object.keys(PRJ_BY_NAME).length){ PRJ_BY_NAME={}; LP.projects.forEach(function(p){ PRJ_BY_NAME[p.name]=p; }); }
    if(!PRJ_PRISTINE){
        PRJ_PRISTINE = { production: LP.production, capacity: LP.capacity, utilisation: LP.utilisation };
    }
    LP.production = PRJ_PRISTINE.production;
    LP.capacity = PRJ_PRISTINE.capacity;
    LP.utilisation = PRJ_PRISTINE.utilisation;
    SCEN_LA = LP.latest_actual ? LP.latest_actual.index : -1;
    SCEN_MON = { prod:{}, capU:{}, capR:{}, util:{} };
    PRJ_PRISTINE.production.views.Monthly.rows.forEach(function(r){ SCEN_MON.prod[r.label]=r.base; });
    PRJ_PRISTINE.utilisation.views.Monthly.rows.forEach(function(r){ SCEN_MON.util[r.label]=r.base; });
    PRJ_PRISTINE.capacity.unrisked.Monthly.rows.forEach(function(r){ SCEN_MON.capU[r.label]=r.base; });
    PRJ_PRISTINE.capacity.risked.Monthly.rows.forEach(function(r){ SCEN_MON.capR[r.label]=r.base; });
    SCEN_N = PRJ_PRISTINE.production.views.Monthly.col_meta.length;
    SCEN_MONTHS = PRJ_PRISTINE.production.views.Monthly.col_meta.map(function(c){ return c.label; });
    SCEN_MONTHS_SHORT = PRJ_PRISTINE.production.views.Monthly.col_meta.map(function(c){ return c.short; });
    // Stable ordering key: pristine full-period production per project — so tree
    // positions don't reshuffle when the range changes or a scenario is applied.
    PRJ_SORTKEY = {};
    PRJ_PRISTINE.production.views.Monthly.rows.forEach(function(r){ var s=0; for(var i=0;i<r.base.length;i++) s+=r.base[i]||0; PRJ_SORTKEY[r.label]=s; });
    // Re-apply a persisted scenario (e.g. after navigating away and back).
    if(scenAnyEdits() && scenMode==='scenario'){ scenRecompute(); scenApplyMode(); }
}
// Shift a monthly series by d months (d>0 = later). Leading gap -> 0; trailing
// (when pulled earlier) carries the last value (plateau continues).
function scenShift(arr, d, n){
    var out = new Array(n);
    for(var i=0;i<n;i++){ var s=i-d; out[i] = (s>=0&&s<n) ? arr[s] : (s>=n ? arr[n-1] : 0); }
    return out;
}
// Transformed monthly {capU,capR,util,prod} for one project (forecast only).
function scenProjectMonthly(name){
    var p = PRJ_BY_NAME[name], la = SCEN_LA, n = SCEN_N;
    var ed = scenEdits[name] || {};
    var capU = (SCEN_MON.capU[name]||[]).slice();
    var capR = (SCEN_MON.capR[name]||[]).slice();
    var util = (SCEN_MON.util[name]||[]).slice();
    var prod = (SCEN_MON.prod[name]||[]).slice();
    // 1) start shift (future projects)
    if(ed.startIdx!=null && p && p.start_idx>=0 && ed.startIdx!==p.start_idx){
        var d = ed.startIdx - p.start_idx;
        capU = scenShift(capU,d,n); capR = scenShift(capR,d,n); util = scenShift(util,d,n);
    }
    // 2) chance of success (scales risked capacity; unrisked unchanged)
    if(ed.cos!=null && p && typeof p.cos==='number' && p.cos>0){
        var k = ed.cos/p.cos;
        for(var i=la+1;i<n;i++) capR[i] = capR[i]*k;
    }
    // 3) utilisation decline rate (ratio form, from the decline-start month). Months
    //    whose utilisation is hardcoded by Palissy (p.hc) stay FIXED — skip them.
    if(ed.declineRate!=null && p && p.decline_start_idx>=0 && typeof p.util_decline==='number' && p.util_decline<1){
        var ds=p.decline_start_idx, oldR=p.util_decline, newR=ed.declineRate, s0=Math.max(ds, la+1);
        var hcSet=(p.hc && p.hc.length)?new Set(p.hc):null;
        for(var i=s0;i<n;i++){ if(util[i]!=null && !(hcSet && hcSet.has(i))) util[i] = util[i]*Math.pow((1-newR)/(1-oldR),(i-ds)/12); }
    }
    // 4) GLOBAL utilisation sensitivity (every project), forecast only, 0% stays 0
    if(scenUtilSens){
        for(var i=la+1;i<n;i++){ if(util[i]!=null && util[i]>1e-12) util[i] = Math.max(0, util[i]+scenUtilSens); }
    }
    // 5) production = risked_cap x util / 12 for the forecast; history frozen
    for(var i=la+1;i<n;i++){ var u=(util[i]==null?0:util[i]); prod[i] = capR[i]*u/12; }
    return { capU:capU, capR:capR, util:util, prod:prod };
}
function scenAnyEdits(){ return scenUtilSens!==0 || Object.keys(scenEdits).length>0; }

/* ---- Re-aggregate a transformed monthly series into a period view via its idx
        map: production = SUM, capacity = END-OF-PERIOD, utilisation = capacity-
        weighted (returns base + weight). ---- */
function scenReaggSum(idx, m){ var o=new Array(idx.length); for(var b=0;b<idx.length;b++){ var s=0,ids=idx[b]; for(var k=0;k<ids.length;k++) s+=m[ids[k]]||0; o[b]=s; } return o; }
function scenReaggEop(idx, m){ var o=new Array(idx.length); for(var b=0;b<idx.length;b++){ var ids=idx[b]; o[b]=ids.length?(m[ids[ids.length-1]]||0):0; } return o; }
function scenReaggUtil(idx, util, w){
    var base=new Array(idx.length), ww=new Array(idx.length);
    for(var b=0;b<idx.length;b++){ var ids=idx[b],num=0,den=0;
        for(var k=0;k<ids.length;k++){ var u=util[ids[k]], x=w[ids[k]]||0; if(u==null) continue; num+=u*x; den+=x; }
        ww[b]=den; base[b]= den>1e-12? num/den : null;
    }
    return {base:base, w:ww};
}
function scenCloneView(orig, newBase){
    var v={}; for(var k in orig) v[k]=orig[k];   // share col_meta/idx/days/... by ref
    v.rows = orig.rows.map(function(r){ var b=newBase(r.label); return b!=null? {label:r.label, base:b} : {label:r.label, base:r.base}; });
    return v;
}
// Rebuild PRJ_SCEN (scenario views) from the per-project transformed monthly series.
function scenRecompute(){
    var mon={}; ALL_DATASETS.lng_projects.projects.forEach(function(p){ mon[p.name]=scenProjectMonthly(p.name); });
    PRJ_SCEN = { production:{views:{}}, capacity:{unrisked:{},risked:{}}, utilisation:{views:{}} };
    var O=PRJ_PRISTINE;
    for(var per in O.production.views){ (function(view){
        PRJ_SCEN.production.views[per]=scenCloneView(view, function(lbl){ return mon[lbl]?scenReaggSum(view.idx, mon[lbl].prod):null; });
    })(O.production.views[per]); }
    [['unrisked','capU'],['risked','capR']].forEach(function(pair){
        var meas=pair[0], key=pair[1];
        for(var per in O.capacity[meas]){ (function(view){
            PRJ_SCEN.capacity[meas][per]=scenCloneView(view, function(lbl){ return mon[lbl]?scenReaggEop(view.idx, mon[lbl][key]):null; });
        })(O.capacity[meas][per]); }
    });
    for(var per in O.utilisation.views){ (function(view){
        var v={}; for(var k in view) v[k]=view[k];
        v.rows = view.rows.map(function(r){ var m=mon[r.label]; if(!m) return {label:r.label,base:r.base,w:r.w};
            var ru=scenReaggUtil(view.idx, m.util, m.capR); return {label:r.label, base:ru.base, w:ru.w}; });
        PRJ_SCEN.utilisation.views[per]=v;
    })(O.utilisation.views[per]); }
}
// Point the live views (read by every renderer + the Overview exports) at scenario
// or pristine. LP === PRJ on the project sub-tabs, so this also drives those.
function scenApplyMode(){
    var LP = ALL_DATASETS.lng_projects; if(!LP || !PRJ_PRISTINE){ scenUpdateBars(); return; }
    var useScen = (scenMode==='scenario') && scenAnyEdits() && PRJ_SCEN;
    var src = useScen ? PRJ_SCEN : PRJ_PRISTINE;
    LP.production = src.production; LP.capacity = src.capacity; LP.utilisation = src.utilisation;
    if(typeof lngExportsInvalidate==='function') lngExportsInvalidate();   // Overview exports rebuild
    scenUpdateBars();
}
// Re-render whichever Global LNG sub-tab is showing.
function scenRerender(){
    if(currentKey==='global_lng'){ if(typeof lngUpdateAll==='function') lngUpdateAll(); return; }
    if(prjSubtab==='capacity') capRender();
    else if(prjSubtab==='utilisation') utilRender();
    else { prjRenderTree(); prjOutlookRender(); }
}
function scenRecomputeAndRender(){ scenRecompute(); scenApplyMode(); scenRerender(); }
// Setters (wired to the UI later)
function scenSetUtilSens(v){ scenUtilSens=v||0; if(scenAnyEdits()&&scenMode==='original') scenMode='scenario'; if(!scenAnyEdits()) scenMode='original'; scenRecomputeAndRender(); }
function scenSetEdit(name, field, val){
    var e=scenEdits[name]||(scenEdits[name]={});
    if(val==null) { delete e[field]; if(Object.keys(e).length===0) delete scenEdits[name]; }
    else e[field]=val;
    if(scenAnyEdits()&&scenMode==='original') scenMode='scenario';
    if(!scenAnyEdits()) scenMode='original';
    scenRecomputeAndRender();
}
function scenSetMode(mode){ scenMode=mode; scenApplyMode(); scenRerender(); }
function scenResetAll(){ var m=document.getElementById('scenResetModal'); if(m){ m.style.display='flex'; } else scenConfirmReset(); }
function scenCancelReset(){ var m=document.getElementById('scenResetModal'); if(m) m.style.display='none'; }
function scenConfirmReset(){
    scenCancelReset();
    scenEdits={}; scenUtilSens=0; scenMode='original'; scenEditMode=false; PRJ_SCEN=null;
    scenApplyMode(); scenRerender();   // bars refresh (Adjust button reverts), views -> pristine
}
...
function scenSetSens(pts){
    ...
    var sl=document.getElementById('scenUtilSensSlider'), nu=document.getElementById('scenUtilSensNum');
    if(sl) sl.value=v; if(nu) nu.value=v;
    scenSetUtilSens(v/100);
}
function scenOnSensSlide(v){ scenSetSens(v); }
function scenOnSensNum(v){ scenSetSens(v); }

/* ---- Per-project edit wrappers (detail panel). null = revert to base value. ---- */
function scenStartOptions(sel){
    var la = SCEN_LA, out='';
    for(var i=la+1;i<SCEN_N;i++) out+='<option value="'+i+'"'+(i===sel?' selected':'')+'>'+(SCEN_MONTHS[i]||i)+'</option>';
    return out;
}
function scenEditCos(name, v){ var p=PRJ_BY_NAME[name], x=parseFloat(v);
    if(isNaN(x)){ scenSetEdit(name,'cos',null); return; }
    var f=Math.max(0,Math.min(1,x/100)); scenSetEdit(name,'cos', Math.abs(f-(p.cos||0))<1e-9?null:f); }
function scenEditDecline(name, v){ var p=PRJ_BY_NAME[name], x=parseFloat(v);
    if(isNaN(x)){ scenSetEdit(name,'declineRate',null); return; }
    var f=x/100; scenSetEdit(name,'declineRate', Math.abs(f-(typeof p.util_decline==='number'?p.util_decline:0))<1e-9?null:f); }
function scenEditStart(name, v){ var p=PRJ_BY_NAME[name], i=parseInt(v);
    scenSetEdit(name,'startIdx', (isNaN(i)||i===p.start_idx)?null:i); }
function scenRevertProject(name){ delete scenEdits[name]; if(!scenAnyEdits()) scenMode='original'; scenRecomputeAndRender(); }
// Delegated handlers for the inline edit inputs/selects + revert links.
document.addEventListener('change', function(e){
    var el=e.target; if(!el||!el.getAttribute) return;
    var f=el.getAttribute('data-scen-edit'); if(!f) return;
    var nm=el.getAttribute('data-name');
    if(f==='cos') scenEditCos(nm, el.value);
    else if(f==='decline') scenEditDecline(nm, el.value);
    else if(f==='start') scenEditStart(nm, el.value);
});
document.addEventListener('click', function(e){
    var el=e.target; if(!el||!el.classList||!el.classList.contains('scen-revert')) return;
    scenRevertProject(el.getAttribute('data-name'));
});

// Total number of distinct changes (each edited field counts; grouped per project).
function scenChangeCount(){
    var n = (scenUtilSens!==0) ? 1 : 0;
    Object.keys(scenEdits).forEach(function(nm){ n += Object.keys(scenEdits[nm]).length; });
    return n;
}
// Hover summary — 3 columns (project | what | value). Global utilisation first;
// ONE ROW PER CHANGE with the project name repeated; rows grouped by project
// (scenEdits keeps insertion order, so a re-edited project stays in place).
function scenChangesSummary(){
    var rows='';
    function r3(k,t,v){ rows+='<div class="scen-pop-row"><span class="scen-pop-k">'+k+'</span><span class="scen-pop-t">'+t+'</span><span class="scen-pop-v">'+v+'</span></div>'; }
    if(scenUtilSens!==0) r3('Global utilisation','Util', (scenUtilSens>0?'+':'')+Math.round(scenUtilSens*100)+'%');
    Object.keys(scenEdits).forEach(function(nm){
        var e=scenEdits[nm], en=prjEsc(nm);
        if(e.cos!=null) r3(en,'CoS', Math.round(e.cos*100)+'%');
        if(e.startIdx!=null) r3(en,'Start', (SCEN_MONTHS_SHORT[e.startIdx]||e.startIdx));
        if(e.declineRate!=null) r3(en,'Decline', (e.declineRate*100).toFixed(1)+'%');
    });
    if(!rows) return '';
    return '<div class="scen-pop-title">Changes in this scenario ('+scenChangeCount()+')</div>'+rows;
}
function scenUpdateBars(){
    var bar=document.getElementById('scenBar'); if(!bar) return;
    ...
    if(nu && parseFloat(nu.value)!==pts) nu.value=pts;
}
...
"""
    ...
    # Scenario bar — always present across the Global LNG sub-tabs (incl. Overview).
    # Hosts the Adjust button, the global utilisation slider (in edit mode), and the
    ...
    html += '        <div class="scen-sens-row">\n'
    html += '          <input type="range" id="scenUtilSensSlider" min="-100" max="100" step="1" value="0" oninput="scenOnSensSlide(this.value)">\n'
    html += '          <input type="number" id="scenUtilSensNum" min="-100" max="100" step="1" value="0" onchange="scenOnSensNum(this.value)">\n'
    html += '          <span class="scen-sens-unit">%</span>\n'
    html += '        </div>\n'
    html += '      </div>\n'
    html += '    </div>\n'
    html += '    <div class="scen-status" id="scenStatus" style="display:none;">\n'
    ...
    # Reset confirmation modal.
    html += '<div class="scen-modal-overlay" id="scenResetModal" style="display:none;">\n'
    html += '  <div class="scen-modal">\n'
    html += '    <div class="scen-modal-title">Reset model?</div>\n'
    html += '    <div class="scen-modal-body">All your changes will be reverted and nothing is stored. Are you sure you want to reset the model?</div>\n'
    html += '    <div class="scen-modal-actions">\n'
    html += '      <button class="scen-modal-no" onclick="scenCancelReset()">No, keep my changes</button>\n'
    html += '      <button class="scen-modal-yes" onclick="scenConfirmReset()">Yes, reset</button>\n'
    html += '    </div>\n'
    html += '  </div>\n'
    html += '</div>\n\n'
    ...
    html += '    <div class="growth-controls lng-head-imports"><span class="lng-block-title">Imports</span></div>\n'
    html += '    <div class="growth-table-container"><table><thead id="lngImpHead"></thead><tbody id="lngImpBody"></tbody></table></div>\n'
    html += '  </div>\n'

    # Imports change table
    html += '  <div class="lng-block">\n'
    html += '    <div class="growth-section">\n'
    html += '      <div class="growth-controls lng-head-imports">\n'
    ...
    # Exports table
    html += '  <div class="lng-block">\n'
    html += '    <div class="growth-controls lng-head-exports"><span class="lng-block-title">Exports</span></div>\n'
    html += '    <div class="growth-table-container"><table><thead id="lngExpHead"></thead><tbody id="lngExpBody"></tbody></table></div>\n'
    html += '  </div>\n'

    # Exports change table
    html += '  <div class="lng-block">\n'
    html += '    <div class="growth-section">\n'
    html += '      <div class="growth-controls lng-head-exports">\n'
